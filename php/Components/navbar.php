...
<header class="z-10 py-4 bg-white shadow-md dark:bg-gray-800">
    <div class="container flex items-center justify-between h-full  mx-auto text-purple-600 dark:text-purple-300">
        GP DU MONDE
        <a class="ml-0 text-lg font-bold text-gray-800 dark:text-gray-200" href="#">
            <img src="../3.png" style=" width : 200px ; height : 100px; "  alt="">
            </a>
        <!-- Mobile hamburger -->
        <button class="p-1 -ml-1 mr-5 rounded-md md:hidden focus:outline-none focus:shadow-outline-purple" @click="toggleSideMenu" aria-label="Menu">
            <svg class="w-6 h-6" aria-hidde n="true" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M3 5a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM3 15a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clip-rule="evenodd"></path>
            </svg>
        </button>
        <!-- Search input -->
        <div class="flex justify-center flex-1 lg:mr-32 ml-56">
            <div class="relative w-full max-w-xl mr-6 focus-within:text-purple-500">
                ...
                <input id="searchInput" class="text-xl py-2 pl-10 pr-2 w-full text-sm text-gray-700 placeholder-gray-600 bg-gray-100 border-2 border-gray-300 rounded-md dark:placeholder-gray-500 dark:focus:shadow-outline-gray dark:focus:placeholder-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:placeholder-gray-500 focus:bg-white focus:border-purple-300 focus:outline-none focus:shadow-outline-purple form-input" type="text" placeholder="Recherche" aria-label="Search" />
            </div>
            <input type="date" class="py-3 border-purple-600 bg-white   text-white-100 px-4 border rounded-lg" value="<?php echo date("Y-m-d"); ?>" class="w-md pl-8 ml-2 pr-2 text-sm text-gray-700 placeholder-gray-600 bg-gray-100 border-0 rounded-md dark:placeholder-gray-500 dark:focus:shadow-outline-gray dark:focus:placeholder-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:placeholder-gray-500 focus:bg-white focus:border-purple-300 focus:outline-none focus:shadow-outline-purple form-input" />
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        let map = L.map('map').setView([51.505, -0.09], 13); // Centrer la carte sur une position initiale
        ...
        map.on('click', function(e) {
            if (!startPoint) {
                startPoint = L.marker(e.latlng).addTo(map).bindPopup('Lieu de départ').openPopup();
                document.getElementById('lieuDepart').value = `${e.latlng.lat}, ${e.latlng.lng}`;
            } else if (!endPoint) {
                endPoint = L.marker(e.latlng).addTo(map).bindPopup('Lieu d\'arrivée').openPopup();
                document.getElementById('lieuArrivee').value = `${e.latlng.lat}, ${e.latlng.lng}`;

                // Calculer la distance une fois les deux points définis
                ...
                document.getElementById('lieuDepart').value = `${e.latlng.lat}, ${e.latlng.lng}`;
                endPoint = null;
                document.getElementById('lieuArrivee').value = '';
            }
        });
    });
